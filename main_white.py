...
def is_light_pixel(pixel):
    return np.mean(pixel) > 200  # Условие для определения светлого пикселя
...
def draw_straight_line(img: np.ndarray, orig_img: np.ndarray, resolution: tuple, noise_level: float = 3.0):
    ...
    step_size = int(line_thickness / 2)
    if step_size == 0:
        step_size = 1
    ...
    line_color = random_color()  # Генерируем случайный цвет для линии
...
def draw_scratch(img: np.ndarray, orig_img: np.ndarray, bezier_points: list, line_thickness: int) -> None:
    line_color = random_color()  # Генерируем случайный цвет для линий
...
def generate_scratch(img: np.ndarray, orig_img: np.ndarray, resolution: tuple, max_length: float,
                     end_brush_range: tuple[float, float], mid_brush_range: tuple[float, float],
    ...
    if use_catmull:
        ...
        draw_scratch(img, orig_img, points, line_thickness)
    else:
        bezier_points = list(bezier(p1, p2, p3, noise_level))
        draw_scratch(img, orig_img, bezier_points, line_thickness)
...
def add_scratches(original_image):
    ...
    for _ in range(random.randint(20, 40)):
        ...
        scratch, bezier_points = generate_scratch(image.copy(), original_image, resolution, max_length=200,
                                                  end_brush_range=(0, 1), mid_brush_range=(1, 5),
            ...
            if check_intersection((lines[i][0], lines[i][-1]),
                ...
                for point in lines[j]:
                    if 0 <= point[0] < image.shape[1] and 0 <= point[1] < image.shape[0]:  # Проверка границ
                        if random.random() < 0.2:  # 20% шанс на белый цвет
                            new_color = (255, 255, 255)  # Полностью белый
                        else:
                            new_color = random_color()  # Используем случайный цвет для царапин
...
def func_white(path: str):
    ...
    new_filename = f"{name}_white{extension}"
...
if __name__ == '__main__':
    # Load the image
    original_image = cv2.imread(r'C:\Users\1\Desktop\Diploma\code\ScratchGenerator\photo1.jpg')
    scratched_image = add_scratches(original_image.copy())
